...
<#if rootSourceType??>
    ...
    /**
     * KNOWN EDGE-CASE – Delete-Recreate with identical Fingerprint:
     *
    * If an object is deleted and a new object with an identical
     * fingerprint (same key attributes) is created, the
     * new object has no correspondence entry. It is therefore treated as
     * "new" in step 5 and transformed again, even though
     * it could conceptually be an update of the deleted object.
     *
     * Consequence: A new object is created in the target model, while
     * the old target object is deleted via the orphaned corrEntry (Partition 4).
     * The result is functionally correct, but
     * cross-references from other objects to the old target object are
     * lost (after the sync, they point to nothing until Phase 6
     * resolves the cross-references again).
     *
     * Approach for future versions: Persistent object IDs
     * (XMI IDs) instead of fingerprinting as the primary identifier.
     * This makes it possible to clearly distinguish "delete-recreate" operations
     * from actual new creations.
     *
     */
    ...
    public static SyncResult sync(
        ...
        // ── Step 1: Partition 1 (src ≠ null, tgt ≠ null) ─────────────────
        for (EObject _ce : _allEntries) {
            ...
            if (!isCoveredByTypeMappingSource(_srcObj)) {
                ...
                // If A and B are handled naturally by mapRoleBasedTypesIncremental/Back.
                continue;
            }
        ...
        // ── Step 2: Partition 2 (src ≠ null, tgt = null) — TypeMapping ──
        for (EObject _ce : _allEntries) {
            EObject _srcObj = CorrespondenceModel.getSourceObject(_ce);
        ...
        // ── Step 3: Partition 3 (src = null, tgt ≠ null) — TypeMapping ──
        for (EObject _ce : _allEntries) {
            EObject _tgtObj = CorrespondenceModel.getTargetObject(_ce);
        ...
        // ── Step 5a: Fuzzy-Match ──────────────────────────────────────────
        // Attempt to link unmatched objects on both sides via fingerprint comparison
        // before creating new objects.
        // Forward: Index of unmatched target objects based on backward fingerprint.
        {
            ...
            // Reverse: Index of unmatched source objects based on the forward fingerprint.
        ...
        // ── Step 5: New objects without corrEntry — TypeMapping ───────────
        for (EObject _srcObj : allSourceObjects(source)) {
            if (!isCoveredByTypeMappingSource(_srcObj)) continue;
            if (corrIndex.containsKey(_srcObj)) continue;
            EObject _newTgt = createNewTargetObject(_srcObj, options);
...
<#if roleBasedTypeMappingModels?has_content>
        // ── Step 1b+2+3+5 for role-based types ───────────────────────────
